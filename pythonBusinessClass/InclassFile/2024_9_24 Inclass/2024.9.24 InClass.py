...
def read_names_from_file(file_name):
    ...
    with open(file_name, "r") as file:
        names = file.read().splitlines() # read each lines and remove the newline
        return names

def count_girls_names(names, girl_name):
    # Count how many gril names in the 'names.txt', and return the repetitive names
    matched_names = [name for name in names if name in girl_names]
    return len(matched_names), matched_names
...
# Count the number of the girl names
girls_name_count , matched_names = count_girls_names(names, girl_names)

print(f"- In the 'names.txt', the amount of the gril names from 'GirlNames.txt': {girls_name_count}")
print("- The repetitve girl names: ")
